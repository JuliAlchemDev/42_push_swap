...
The Chunk Sort algorithm divides the original stack a into `ceil(√n)` sub-stacks each of length `floor(√n)` or less.
The `ceil(√n)` sub-stacks are created by pushing one-by-one the elements of the stack `a` into stack `b`, the calls to function `pb` are intervalled by the rotations `rb` and `rrb` to make each substack sorted while keeping them separated.

The last four or less elements of `a` are sorted with the function `sort_a_small`.
